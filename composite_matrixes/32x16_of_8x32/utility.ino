...
// для матрицы размером 32х16, собранной из двух размером 8х32
// https://editor.soulmatelights.com/gallery/1188-xy-32x16

uint16_t XY(uint8_t x, uint8_t y)
{
 if (y<8) // 1st matrix 8x32
   { 
   ...
   {
     y = y - 8; // shift of 0:0 coordinate
     if (x % 2 == 1) // odd columns
     {
       return  (8*32) + (32 - x - 1) * 8 + y;
     }
     //else // even columns
     {
       return  (8*32) + (32 - x - 1) * 8 + 8 - y - 1;
     }
   }
}
